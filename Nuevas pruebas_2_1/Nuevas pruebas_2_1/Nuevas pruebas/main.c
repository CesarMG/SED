...
uint8_t FLAG_EINT0_PULSO;			// Flag para habilitar disparo
uint8_t FLAG_EINT_SERVO;			// Flag para habilitar mov. manual
uint8_t FLAG_AUTO;						// Flag modo automático
uint8_t FLAG_DETEC;						// Flag modo detecccion obs.
uint8_t FLAG_ONLINE;					// Flag modo online
uint8_t FLAG_OFFLINE;					// Flag modo offline
uint8_t DAC_ON = 0;						// Flag activacion DAC
uint8_t FLAG_MENU = 0;				// Flag iniciar menu
uint8_t FLAG_init_timer = 0;	// Flag inicializacion timers
...
float temperatura_global;  // Variable temperatura LM35
float temperaturaI2C = 0;  // Variable temperatura I2C
int		temp				= 0;
float temp_I2C		= 30;
...
char buffer_umbral[30] ;   // Buffer para enviar umbral
...
int main()
{
  ...
  fillScreen(BLACK);			// Pintamos la pantalla de negro
	...
	config_pines();					// Configuramos pines
	config_externas();			// Configuramos interrupciones externas
	config_pwm1();					// Configuracion PWM servo
	config_ADC();						// Configuracion ADC
	config_DAC();						// Configuracion DAC
	config_TIMER0();				// Configuracion TIMER0
	config_TIMER1();				// Configuracion TIMER1
	config_TIMER2();				// Configuracion TIMER2
	config_TIMER3();				// Configuracion TIMER3
	genera_muestras(N_muestras);	// Generamos el seno
	config_DS1621();				// Configuracion sensor i2c
	config_prioridades();		// Configuracion prioridades
	ptr_rx = buffer;				// inicializar puntero en el buffer
	uart0_init(9600);				// inicializar uart a 9600 baudios
	...
	{	
		switch(mode)
		{
			...
			break;
			case OFFLINE_M:	// Modo 11: Manual (offline)
			...
			case OFFLINE_M_U:	   // Modo manual - medida unica (offline)
				if(FLAG_MENU == 0)
				{
					...
					FLAG_EINT0_PULSO = 1;		// Habilitamos disparo unico con ISP
					FLAG_EINT_SERVO  = 1;		// Habilitamos movimiento con KEY1 y KEY2
					FLAG_OFFLINE = 1;				// Modo Offline
				}
			...
			case OFFLINE_M_C: // Modo manual - medidas continuas (offline)
				...
				if(FLAG_ISP)								      // Boton ISP pulsado
					FLAG_DISPARO_CONTINUO = 1;      // Habilitamos disparo continuo
